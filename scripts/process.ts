...
;(async () => {
  ...
  while ((match = regex.exec(readme.toString()))) {
    data[match[3]] = {
      name: match[1],
      count: +match[2]
    }
  ...
  for (const file of files) {
    ...
    data[name].data.features.forEach((i: any) => {
    ...
    // calc center
    const coordinates = data[name].data.features.map((i: any) => i.geometry.coordinates)
    data[name].center = coordinates
      .reduce((a: any, b: any) => [a[0] + b[0], a[1] + b[1]], [0, 0])
      .map((i: any) => i / coordinates.length)
  }
